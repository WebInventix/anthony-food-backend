...
const getAdminOrders = async (req,res) => {
   ...
   try {
    ...
    if(store && vendor)
    {
        orders = await Orders.find({store_id:store, vendor_id:vendor}).populate('product_id')
    }
    else if(store && !vendor)
    {
        orders = await Orders.find({store_id:store}).populate('product_id')
    }
    else
    {
        orders = await Orders.find().populate('product_id')
    }
